...
alias ssh2host="$SSH_PATH -o UserKnownHostsFile=/dev/null -o ConnectionAttempts=180 -o StrictHostKeyChecking=no -o PreferredAuthentications=password -o LogLevel=ERROR -o TCPKeepAlive=no -o ServerAliveInterval=45 fuse@$IP_ROOT"
# alias to connect to the ssh server exposed by JBoss Fuse. uses sshpass to script the password authentication
alias ssh2fabric="sshpass -p admin $SSH_PATH -p 8101 -o ConnectionAttempts=180 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o PreferredAuthentications=password -o LogLevel=ERROR -o TCPKeepAlive=no -o ServerAliveInterval=45 admin@$IP_ROOT"
# alias to connect to the ssh server exposed by JBoss Fuse. uses sshpass to script the password authentication
alias ssh2fab02="sshpass -p admin $SSH_PATH -p 8101 -o ConnectionAttempts=180 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o PreferredAuthentications=password -o LogLevel=ERROR -o TCPKeepAlive=no -o ServerAliveInterval=45 admin@$IP_FAB02"
alias ssh2fab03="sshpass -p admin $SSH_PATH -p 8101 -o ConnectionAttempts=180 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o PreferredAuthentications=password -o LogLevel=ERROR -o TCPKeepAlive=no -o ServerAliveInterval=45 admin@$IP_FAB03"
...
while [[ Xtrue != X$(curl --cookie-jar $COOKIEFILENAME -X POST --data "username=$USERNAME&password=$PASSWORD" $FMC_URL/rest/system/login 2> /dev/null) ]]; do echo "not logged yet"; sleep 3s ; done 
...
while [[ Xtrue != X$(ssh2fabric 'dev:wait-for-service -t 30000  "(&(objectClass=javax.servlet.ServletContext)(osgi.web.symbolicname=org.fusesource.fabric.fabric-jolokia)(osgi.web.version=7.2.0.redhat-060))" ') ]]; do echo "not provisioned yet"; sleep 3s ; done 


ssh2fabric "container-list"

# set new version before provisioning
ssh2fabric "fabric:version-set-default 1.2"
